...
#Grocery Store has mose of its data points around lower sales values compated to other categories.
...
combi[,Item_Visibility := log(Item_Visibility + 1)] # log + 1000 to avoid division by 0.
...
summary(lasso_linear_reg_model)

# Ridge Regression
set.seed(1236)
my_control <- trainControl(method = "cv", number = 5)
Grid <- expand.grid(alpha = 0, lambda = seq(0.001,0.1,by = 0.0002))
ridge_linear_reg_model <- train(x = train[, -c("Item_Identifier", "Item_Outlet_Sales")], 
                                y = train$Item_Outlet_Sales, method = "glmnet", 
                                trControl = my_control, tuneGrid = Grid)
summary(ridge_linear_reg_model)
names(ridge_linear_reg_model)
ridge_linear_reg_model$results

#making predictions on test data
submission$Item_Outlet_Sales <- predict(linear_reg_model, test[,-c("Item_Identifier")])
#Lasso predictions
submission$Item_Outlet_Sales_1 <- predict(lasso_linear_reg_model, test[,-c("Item_Identifier")])
# Ridge Predictions
submission$Item_Outlet_Sales_2 <- predict(ridge_linear_reg_model, test[,-c("Item_Identifier")])

write.csv(submission, "Linear_Reg_submit.csv", row.names = FALSE)
write.csv(submission, "Lasso_Reg_submit.csv", row.names = FALSE)
write.csv(submission, "ridge_Reg_submit.csv", row.names = FALSE)

# calculating RMSE for in sample data(train data) after building the model (1127.269)
SSE_train_data <- sum(linear_reg_model$residuals ^2)
SSE_train_data
RMSE_train <- sqrt(SSE_train_data/nrow(train))
RMSE_train

#RMSE for Lasso regression (1129.53)
names(lasso_linear_reg_model)
lasso_linear_reg_model$results

##RMSE for Ridge regression (1134.585)
ridge_linear_reg_model$results
...
plot(rf_mod)
#variable importance
plot(varImp(rf_mod)) # the features "Item_MRP", "price_per_unit_wt", "Item_MRP_Clusters" are
#amongst the top important variables.Also best score achieved at mtry = 6 & min.node.size = 20.
names(rf_mod)
##RMSE for Random Forest at mtry = 6, min.node.size = 20 (1090.396)
rf_mod$results

# Random Forest Predictions (making predictions on test data)
submission$Item_Outlet_Sales_3 <- predict(rf_mod, test[,-c("Item_Identifier")])

write.csv(submission, "rf_Reg_submit.csv", row.names = FALSE)
